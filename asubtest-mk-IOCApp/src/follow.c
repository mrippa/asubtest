 ...
 */
long initFollowA (aSubRecord *prec)
{
    *(long *)prec->vale = 0;	/* first array at offset zero */
    printf("initFollowA called");
    return(0);
}
 ...
 *  Inputs:
 *  -------
 *  prec->a = TCS Logging switch		LONG
 *  prec->b = Az. lower limit			DOUBLE
 *  prec->c = Az. upper limit			DOUBLE
 *  prec->d = El. lower limit			DOUBLE
 *  prec->e = El. upper limit			DOUBLE
 *  prec->j = Array of 5 from TCS		DOUBLE
 *
 *  Outputs:
 *  --------
 *  prec->vala = 5 element array from TCS	DOUBLE
 *  prec->valb = 5 element array from TCS	DOUBLE
 *  prec->valc = 5 element array from TCS	DOUBLE
 *  prec->vald = Which of above is latest	LONG
 *  prec->vale = One more than above		LONG
 *  prec->valf = Azimuth demand from TCS	DOUBLE
 *  prec->valg = Elevation demand from TCS	DOUBLE
 *
...
static long FollowA (aSubRecord *prec)
{
    long   logSwitch;
    double azLower;
    double azUpper;
    ...
    /* Get input fields.
    */
    logSwitch = *(long *)   prec->a;	/* log flag */
    azLower   = *(double *) prec->b;	/* az lower limit */
    azUpper   = *(double *) prec->c;	/* az upper limit */
    elLower   = *(double *) prec->d;	/* el lower limit */
    elUpper   = *(double *) prec->e;	/* el upper limit */
    inptr     = (double *) prec->j;	/* pointer to input array from TCS */
    ...
    if( logSwitch )
    {
        ...
        tcsjj++;
        if( tcsjj == LOG_LIMIT )
            *(long *) prec->a = 0;    /* Switch-Off logging */
    }
#endif
    ...
    */
    *(long *) prec->vale %= 3;
    switch (*(long *)prec->vale)
    {
        case 0:
            outptr = (double *) prec->vala;
            prevptr = (double *) prec->valb;
            break;

        case 1:
            outptr = (double *) prec->valb;
            prevptr = (double *) prec->valc;
            break;

        case 2:
            outptr = (double *) prec->valc;
            prevptr = (double *) prec->vala;
            break;
     ...
     */
    *(double *) prec->valf = *(prevptr+3);	/* az */
    *(double *) prec->valg = *(prevptr+4);	/* el */
 ...
 * Inputs:
 * -------
 *    prec->a = Array of 5 doubles.
 *    prec->b = Array of 5 doubles.
 *    prec->c = Array of 5 doubles.
 *    prec->d = Which one of the above was most recent?    LONG.
 *    prec->e = Follow Flag                                LONG.
 *    prec->f = Current Track Id                           DOUBLE.
 *    prec->g = Azimuth Current Position                   DOUBLE.
 *    prec->h = Azimuth Current Velocity                   DOUBLE.
 *    prec->i = Internal use:				    LONG.
 *               Previous recent sample
 *    prec->j = Not used				    LONG.
 *    prec->k = Elevation Current Position                 DOUBLE.
 *    prec->l = Elevation Current Velocity                 DOUBLE.
 *    prec->m = Azimuth Current Max. Velocity              DOUBLE.
 *    prec->n = Elevation Current Max. Velocity            DOUBLE.
 *    prec->o = Azimuth Current Max. Acceleration          DOUBLE.
 *    prec->p = Elevation Current Max. Acceleration        DOUBLE.
 *    prec->q = Handshake bit from Az. PMAC motion program DOUBLE.
 *    prec->r = Handshake bit from El. PMAC motion program DOUBLE.
 *    prec->s = Trajectory Calculation Mode                LONG.
 *    prec->t = az Pmac Demand Position                    DOUBLE.
 *    prec->u = el Pmac Demand Position                    DOUBLE.
 *
 * Outputs:
 * --------
 *    prec->vala = Array of NUM_EXTRAP Azimuth Positions           DOUBLE.
 *    prec->valb = Array of NUM_EXTRAP Azimuth Velocities          DOUBLE.
 *    prec->valc = Array of NUM_EXTRAP Elevation Positions         DOUBLE.
 *    prec->vald = Array of NUM_EXTRAP Elevation Velocities        DOUBLE.
 *    prec->vale = Mask for fanout                         LONG.
 *                  1  (Link 1) = Set trackId.
 *                  2  (Link 2) = Fill Az. buffer.
 *                  4  (Link 3) = Fill El. buffer.  
 *                  8  (Link 4) = Set time interrupt.
 *                  16 (Link 5) = Set Car Error.
 *    prec->valf = Time for external Bancomm Trigger       DOUBLE.
 *    prec->valg = TrackId                                 DOUBLE.
 *    prec->valh = Current Network time delay (sec)        DOUBLE.
 *    prec->vali = Number of samples missed                LONG.
 *    prec->valj = Fill bottom/top of Az. buffer (1/2)     LONG.
 *    prec->valk = Fill bottom/top of El. buffer (1/2)     LONG.
 *    prec->vall = Error string                            LONG.
 *
 */

long Tracking (aSubRecord *prec)
{
    double arrayA[5];
    double arrayB[5];
    double arrayC[5];
    ...
    */
    ...
    follow          = *(long *) prec->e;
    /*#if 0 */
    trackid         = *(double *) prec->f;
    /*#endif */
    precent         = *(long *) prec->i;
    azHandShake     = (long)(*(double *) prec->q);
    elHandShake     = (long)(*(double *) prec->r);

    azCurrentPos    = *(double *) prec->g;
    azCurrentVel    = *(double *) prec->h;
    elCurrentPos    = *(double *) prec->k;
    elCurrentVel    = *(double *) prec->l;

    azCurrentMaxVel = *(double *) prec->m;
    elCurrentMaxVel = *(double *) prec->n;

    azCurrentMaxAcc = *(double *) prec->o;
    elCurrentMaxAcc = *(double *) prec->p;

    trajectoryMode  = *(long *) prec->s;
    azPmacDemandPos = *(double *) prec->t;
    elPmacDemandPos = *(double *) prec->u;
    ...
    if (follow) 
    {
                ...
                (elHandShake != prevElHandShake))
        {
            ...
            */
             ...
             */
            if (error)
                sprintf (prec->vall, "timeNow returned %ld", error);
            else
            {
...
#endif
                            mask = mask | 8;
                            *(double *)prec->valf = 60.0*60.0*hmsf[0] +
                                60.0 * hmsf[1] +
                                hmsf[2] +
                ...
                if (!error)
                {
                    ...
                    *(double *) prec->valg = newid;
                    ...
                    if (azHandShake != prevAzHandShake)
                    {
                         ...
                         */
                        if (azHandShake == 0)
                            *(long *) prec->valj = 2; /* Fill top-half */
                        else
                            *(long *) prec->valj = 1; /* Fill bottom-half */
                        /*if (!slewFlag)*/	
                        mask = mask | 2;
                    ...
                    */
                    if (elHandShake != prevElHandShake)
                    {
                        ...
                        if (elHandShake == 0)
                            *(long *)prec->valk = 2; /* Fill top-half */
                        else
                            *(long *)prec->valk = 1; /* Fill bottom-half */
    ...
    *(long *) prec->vale = mask;
    memcpy (prec->vala, azPos, NUM_EXTRAP * sizeof(double));
    memcpy (prec->valb, azVel, NUM_EXTRAP * sizeof(double));
    memcpy (prec->valc, elPos, NUM_EXTRAP * sizeof(double));
    memcpy (prec->vald, elVel, NUM_EXTRAP * sizeof(double));
    *(double *)prec->valp = azPos[0];
    *(double *)prec->valq = elPos[0];
 ...
 * chain of processing.
 */
long trackingFO (aSubRecord *prec)
{
    double *dptr;
    int    i;
    ...
    for (i=0; i<NUM_EXTRAP; i++)
    {
        ...
        {
            case 0:
                *(double *)prec->vala = *(dptr++);
                break;

            case 1:
                *(double *)prec->valb = *(dptr++);
                break;

            case 2:
                *(double *)prec->valc = *(dptr++);
                break;

            case 3:
                *(double *)prec->vald = *(dptr++);
                break;

            case 4:
                *(double *)prec->vale = *(dptr++);
                break;

            case 5:
                *(double *)prec->valf = *(dptr++);
                break;

            case 6:
                *(double *)prec->valg = *(dptr++);
                break;

            case 7:
                *(double *)prec->valh = *(dptr++);
                break;

            case 8:
                *(double *)prec->vali = *(dptr++);
                break;

            case 9:
                *(double *)prec->valj = *(dptr++);
                break;

#if NUM_EXTRAP>10
            case 10:
                *(double *)prec->valk = *(dptr++);
                break;

            case 11:
                *(double *)prec->vall = *(dptr++);
                break;

            case 12:
                *(double *)prec->valm = *(dptr++);
                break;

            case 13:
                *(double *)prec->valn = *(dptr++);
                break;

            case 14:
                *(double *)prec->valo = *(dptr++);
                break;

            case 15:
                *(double *)prec->valp = *(dptr++);
                break;

            case 16:
                *(double *)prec->valq = *(dptr++);
                break;

            case 17:
                *(double *)prec->valr = *(dptr++);
                break;

            case 18:
                *(double *)prec->vals = *(dptr++);
                break;

            case 19:
                *(double *)prec->valt = *(dptr++);
                break;
#endif
            default:
 ...
 *    prec->d = Which one of the above was most recent?    LONG.
 *    prec->e = Follow Flag                                LONG.
 *    prec->f = Trajectory Calculation Mode                LONG.
 *    prec->g = Azimuth Current Position                   DOUBLE.
 *    prec->h = Elevation Current Position                 DOUBLE.
 *    prec->i = Azimuth Current Max. Velocity              DOUBLE.
 *    prec->j = Elevation Current Max. Velocity            DOUBLE.
 *    prec->k = Azimuth Current Max. Acceleration          DOUBLE.
 *    prec->l = Elevation Current Max. Acceleration        DOUBLE.
 *
 * Outputs:
 * --------
 *    prec->vala = Array of 5 doubles.                     DOUBLE.
 *    prec->valb = Array of 5 doubles.                     DOUBLE.
 *    prec->valc = Array of 5 doubles.                     DOUBLE.
 *    prec->vald = Which of above is latest	            LONG
 *
 */

long TrajecCalc (aSubRecord *prec)
{
    double arrayA[5];
    double arrayB[5];
    double arrayC[5];
    ...
    */
    recent          = *(long *) prec->d;
    follow          = *(long *) prec->e;
    trajectoryMode  = *(long *) prec->f;

    azCurrentPos    = *(double *) prec->g;
    elCurrentPos    = *(double *) prec->h;

    azCurrentMaxVel = *(double *) prec->i;
    elCurrentMaxVel = *(double *) prec->j;

    azCurrentMaxAcc = *(double *) prec->k;
    elCurrentMaxAcc = *(double *) prec->l;
...
*/
void logTracking (aSubRecord *prec)
{
    long axis;
 ...
 *
 *    prec->a    = logging flag                        LONG.
 *    prec->vala = Mask for fanout                     LONG.
 */
void waitLogTrackingEnd (aSubRecord *prec)
{

    loggingFlag = *(long *)   prec->a;	/* log flag */
...
/* initFollowA2 - for genSubRecord */
long initFollowA2 (struct genSubRecord *pgsub)
{
    *(long *)pgsub->vale = 0;	/* first array at offset zero */
    printf("initFollowA2 called\n");
    return(0);
}
 ...
 * Test genSub
 * */
static long FollowA2 (struct genSubRecord *pgsub)
{
    long   logSwitch;
    double azLower;
    double azUpper;
...
epicsRegisterFunction(FollowA2);
